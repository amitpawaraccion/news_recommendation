...
# driver = webdriver.Chrome("/home/amit_pawar/webscrapping/assets/chromedriver")
# Load news-section urls in a list from text document that contains the list of The Economic Times section urls
# you want to scrape
# file_path = 'C:/Users/apkumar/Downloads/web_scrapper/ET_urls.txt'
# urls = [line.rstrip('\n') for line in open(file_path)]

urls = [
        
        'https://economictimes.indiatimes.com/tech/software','https://economictimes.indiatimes.com/industry',
        'https://www.thehindu.com/','https://www.hindustantimes.com/','https://www.ndtv.com/', 
        'https://www.ndtv.com/', 'https://indianexpress.com/', 'https://www.indiatoday.in/', 'https://www.news18.com/'
        'https://www.msn.com/en-in/feed/interest/news?ocid=msedgntp'
...
for url in urls:
    ...
    while len(tag_dup)<=2000 and len(tag_dup)>=page_break:
        source = driver.page_source
        soup = BeautifulSoup(source,'html.parser')
        tags = soup.find_all('div',{'class':'story-box clearfix'})
...
# 2nd Section - Extracting date and full news text from the Links scraped previously
    ...
    for link in title_df['Link']:
        ...
        try:
            if news_soup.find('div',{'class':'dtc vam artByline'}).text:
                date = news_soup.find('div',{'class':'dtc vam artByline'}).find('time').text
        ...
        try:
            if news_soup.find('div',{'class':'artText'}).text:
                article = news_soup.find('div',{'class':'artText'}).text
            else:
                print('')
        except AttributeError:
            article = 'N/A'
    ...
    out.to_csv('/home/amit_pawar/webscrapping/output/news'+str(url_index)+'.csv')
